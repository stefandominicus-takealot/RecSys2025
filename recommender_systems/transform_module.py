...
def preprocessing_fn(inputs: dict[str, tf.Tensor]) -> dict[str, tf.Tensor]:
    ...
    for feature in [
        CustomerFeatures.ID,
        ProductFeatures.ID,
        # TODO[Workshop]: Consider transforming other categorical features
        # ProductFeatures.BRAND,
    ]:
        deferred_vocab = tft.vocabulary(
            inputs[feature], vocab_filename=feature, reserved_tokens=["MASK"]
        )
        outputs[xf(feature)] = tft.apply_vocabulary(
            inputs[feature],
            deferred_vocab_filename_tensor=deferred_vocab,
            default_value=0,  # MASK
        )
    ...
    for feature in [
        ...
        # TODO[Workshop]: Consider transforming numerical features
    ...
    for feature in [
        ...
        # TODO[Workshop]: Consider transforming text features
    ...
    # feature that is 1.0 for positive reviews (rating >= 4 stars) and 0.0 for negative
